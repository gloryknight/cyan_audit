...
declare
    my_uid    integer;
begin
    my_uid := @extschema@.fn_get_config('_uid')::integer;
...
language sql stable
as $_$
    SELECT (@extschema@.fn_get_config('_last_txid'))::bigint;
 $_$;
...
-- fn_set_last_txid
CREATE OR REPLACE FUNCTION @extschema@.fn_set_last_txid
(
...
begin
    select @extschema@.fn_get_config('user_table'),
           @extschema@.fn_get_config('user_table_uid_col'),
           @extschema@.fn_get_config('user_table_email_col')
      into my_user_table,
...
begin
    select @extschema@.fn_get_config('user_table'),
           @extschema@.fn_get_config('user_table_uid_col'),
           @extschema@.fn_get_config('user_table_username_col')
      into my_user_table,
           my_user_table_uid_col,
...
begin
    ...
    for my_table_name in
        ...
        select @extschema@.fn_get_partitions_over_size_limit( in_keep_size_gb )
         UNION
        select @extschema@.fn_get_partitions_older_than_interval( in_keep_interval );
    loop
...
CREATE OR REPLACE FUNCTION @extschema@.fn_get_partitions_over_size_limit( in_size_gb
