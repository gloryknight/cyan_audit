...
drop function if exists public.fn_drop_audit_event_log_trigger(in_table_name character varying)
drop function if exists public.fn_get_or_create_audit_data_type(in_type_name character varying)
drop function if exists public.fn_get_or_create_audit_field(in_table_name character varying, in_column_name character varying, in_active boolean)
drop function if exists public.fn_get_or_create_audit_transaction_type(in_label character varying)
drop function if exists public.fn_get_or_create_system_audit_field(in_system integer, in_audit_field integer)
...
drop function if exists public.fn_update_audit_event_log_trigger_on_table(in_table_name character varying)
...
alter table audit_log.tb_audit_event 
    rename column entity to uid;
alter table audit_log.tb_audit_event 
    rename column transaction_id to txid;
alter table audit_log.tb_audit_event 
    rename column process_id to pid;
alter table audit_log.tb_audit_event 
